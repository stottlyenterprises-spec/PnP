...
export async function GET(req:NextRequest){
 const disconnectAll=req.nextUrl.searchParams.get("all")==="1",nativeReturn=req.nextUrl.searchParams.get("native")==="1",requested=Number(req.nextUrl.searchParams.get("slot")),active=await activeGoogleSlot(),slot=Number.isInteger(requested)&&requested>=0&&requested<=4?requested:active,accounts=await readGoogleAccounts(),remaining=disconnectAll?[]:accounts.filter(account=>account.slot!==slot),nextActive=remaining[0]?.slot??0,res=NextResponse.redirect(nativeReturn?`deeds://open?view=data&google=disconnected`:new URL(disconnectAll?"/?account=choose":"/?google=disconnected",req.nextUrl.origin));
