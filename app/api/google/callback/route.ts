...
export async function GET(req:NextRequest){
 ...
 const body=await token.json(),profileResponse=await fetch("https://www.googleapis.com/oauth2/v2/userinfo",{headers:{Authorization:`Bearer ${body.access_token}`},cache:"no-store"}),profile=profileResponse.ok?await profileResponse.json():{},storedAccounts=await readGoogleAccounts(),accounts=storedAccounts.length?storedAccounts:slot>0&&(jar.get("google_access")?.value||jar.get("google_refresh")?.value)?[{slot:0,email:"Connected Google account"}]:[],email=String(profile.email||`Google account ${slot+1}`),nextAccounts=[...accounts.filter(account=>account.slot!==slot),{slot,email}].sort((a,b)=>a.slot-b.slot),res=NextResponse.redirect(new URL("/?google=connected",app));
