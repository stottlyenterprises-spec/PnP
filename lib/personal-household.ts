export const PERSONAL_HOUSEHOLD_MIGRATION = "personal-household-areas-v3";
const PREVIOUS_PERSONAL_HOUSEHOLD_MIGRATIONS = new Set(["personal-household-areas-v1","personal-household-areas-v2"]);
...
const areaFromTitle=(title:string)=>{const value=key(title);if(/playhouse|outdoor play|outdoor toys/.test(value))return"Outdoor Play Area";if(/pool|spa|skimmer/.test(value))return"Pool & Spa";if(/back patio|lanai|outdoor grill|patio table|patio chair/.test(value))return"Back Patio & Lanai";if(/garage/.test(value))return"Garage";if(/refrigerator|pantry|kitchen|dishwasher|garbage disposal|microwave|oven|stovetop|hood vent|coffee maker|toaster|air fryer/.test(value))return"Kitchen";if(/bathroom|toilet|vanity|shower|bathtub/.test(value))return"Bathrooms";if(/bedroom|mattress|bedding|make beds|closet/.test(value))return"Bedrooms";if(/living room|family room|dining room/.test(value))return"Living Spaces";if(/office|computer|desk/.test(value))return"Office";if(/washing machine|dryer|laundry|water heater|hvac|utility|ac line/.test(value))return"Laundry & Utility";if(/exterior|gutter|roof|driveway|porch|flower bed|landscap|tree|bush/.test(value))return"Exterior";if(/whole house|ceiling fan|air vent|baseboard|window track|under furniture|curtain|carpet|wood furniture|return items|floor pickup|empty trash/.test(value))return"Whole House";return null};
const areaFromCategory=(category:string)=>{const value=key(category);if(/outdoor play/.test(value))return"Outdoor Play Area";if(/pool|spa/.test(value))return"Pool & Spa";if(/patio|lanai/.test(value))return"Back Patio & Lanai";if(/garage/.test(value))return"Garage";if(/kitchen/.test(value))return"Kitchen";if(/bathroom/.test(value))return"Bathrooms";if(/bedroom/.test(value))return"Bedrooms";if(/living/.test(value))return"Living Spaces";if(/office/.test(value))return"Office";if(/laundry|utility/.test(value))return"Laundry & Utility";if(/exterior/.test(value))return"Exterior";if(/whole house/.test(value))return"Whole House";return"Other Household"};
// The task title is authoritative. The old category is only a fallback for
// genuinely generic titles, otherwise a bad category can perpetuate itself.
const areaFor=(title:string,category="")=>areaFromTitle(title)||areaFromCategory(category);
...
export function upgradePersonalHousehold<T extends TaskLike,C extends CategoryLike,L extends ListLike>(input:{tasks:T[];categories:C[];lists:L[];migrations:string[];today:string}){
 ...
 const householdIds=new Set(household.map(list=>list.id)),oldCategories=new Map(input.categories.map(category=>[category.id,category])),alreadyOrganized=input.migrations.some(migration=>PREVIOUS_PERSONAL_HOUSEHOLD_MIGRATIONS.has(migration));
